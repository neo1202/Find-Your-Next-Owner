            ...
            <div className="mt-4 grid grid-cols-2 place-content-evenly place-items-center gap-x-8 gap-y-4 border-t border-gray-200 p-9 pt-2 lg:grid-cols-3">
                {posts && posts.map((post) => (
                    <a key={post.id} href={`/posts/${post.id}`}>
                        <article className=" flex-start delay-50 relative flex h-[28rem] max-w-xl flex-col rounded-xl bg-teal-50 p-2 shadow-xl transition ease-in-out hover:-translate-y-1 hover:scale-105 hover:bg-teal-300 md:h-[32rem]">
